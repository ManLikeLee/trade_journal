...
import { useTrade, TradeNote } from '@/hooks/useTrades';
        ...
        <div className="flex items-center justify-between px-3.5 py-3 border-b border-border shrink-0">
          {trade ? (
            <div className="flex items-center gap-2.5">
              <div className={cn('w-1 h-7 rounded-full', trade.direction === 'BUY' ? 'bg-profit' : 'bg-loss')} />
              <div>
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-[11px] text-muted-foreground">{Number(trade.lotSize).toFixed(2)} lots</span>
                </div>
                <p className="text-[10.5px] text-muted-foreground">{trade.source}</p>
              </div>
            </div>
          ) : (
            ...
            <div className="p-3.5 space-y-3.5">
              ...
              {trade.pnl !== null && (
                <div className={cn('p-3 text-center', pnl >= 0 ? 'ui-surface-profit' : 'ui-surface-loss')}>
                  ...
                  {rr && (
                    <p className="text-xs mt-1 opacity-80">R:R  1:{rr.toFixed(2)}</p>
                  )}
                </div>
              )}
              ...
              <div className="grid grid-cols-2 gap-2">
                {[
                  ...
                  <div key={label} className="rounded-md bg-muted/45 border border-border p-2.5">
              ...
              {trade.tradeTags?.length > 0 && (
                <div>
                  <p className="text-[10.5px] font-medium text-muted-foreground mb-1.5 flex items-center gap-1.5">
              ...
              {/* Notes */}
              <div>
                <p className="text-[10.5px] font-medium text-muted-foreground mb-2 flex items-center gap-1.5">
                  <MessageSquare className="w-3.5 h-3.5" /> Trade Journal Notes
                </p>
                {trade.notes?.map((note: TradeNote) => (
                  <div key={note.id} className="mb-2 rounded-md border border-border bg-muted/20 p-2.5">
                    {note.emotion && (
                      <span className={cn('text-[10px] px-1.5 py-0.5 rounded font-medium mb-1.5 inline-block',
                        EMOTION_COLORS[note.emotion] ?? 'bg-muted')}>
                        {note.emotion}
                      </span>
                    )}
                    <p className="text-xs text-foreground">{note.content}</p>
                    <p className="text-[10.5px] text-muted-foreground mt-1.5">{formatDateTime(note.createdAt)}</p>
                  </div>
                ))}
                ...
                <div className="mt-2.5 space-y-2">
                  <select
                    ...
                    className="ui-select h-8 text-[11px] px-2"
                  >
                  ...
                  <textarea
                    ...
                    rows={3}
                    className="ui-textarea text-xs resize-none"
                  />
                  <button
                    onClick={addNote}
                    disabled={!noteText.trim() || savingNote}
                    className="ui-btn-primary w-full h-8"
                  >
